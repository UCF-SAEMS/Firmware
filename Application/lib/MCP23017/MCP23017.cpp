...
MCP23017::MCP23017(I2C_Handle& bus, uint8_t address) {
	_deviceAddr = address;
	_bus = &bus;
}
...
{
    uint8_t ibuf[2] = { static_cast<uint8_t>(reg), value};
...
{
    uint8_t ibuf[3] = { static_cast<uint8_t>(reg), portA, portB };
...
uint8_t MCP23017::readRegister(MCP23017Register reg)
{
  uint8_t ibuf = 0x00;
    ...
    _txBuffer[0] = static_cast<uint8_t>(reg);

    _i2cTransaction.writeCount = 1;
    _i2cTransaction.writeBuf = _txBuffer;
    _i2cTransaction.readCount = 1;
    _i2cTransaction.readBuf = &ibuf;
    _i2cTransaction.slaveAddress = _deviceAddr;
